	...
	class VisitorsVoiceMetabox {
		...
		public function add_meta_box() {
			...
			add_meta_box(
				...
				,__( 'Visitors Voice', self::LANG )			# Title of the metabox the edit screen section, visible to user
...
?>				
					...
					<table class="display" id="searchterms_refinements_table">
						<thead>
							<tr>
								<th><?php _e("Initial search terms", "visitorsvoice"); ?><abbr title="<?php _e("Initial search term that was later refined.", "visitorsvoice") ?>">?</abbr></th>
								<th><?php _e("Refinements", "visitorsvoice"); ?><abbr title="<?php _e("Refined search term that led to a click to this page.", "visitorsvoice") ?>">?</abbr></th>
								<th><?php _e("Users", "visitorsvoice"); ?></th>
								<?php if ($visitorsvoiceplugin->get_keywords_field()): ?>
									<th><?php _e("Add initial search term to keywords", "visitorsvoice"); ?> <abbr title="<?php _e("Add the search term as a keyword to the keyword post meta field.", "visitorsvoice") ?>">?</abbr></th>
								<?php endif ?>
							</tr>
						</thead>
						<tbody>
<?php					
							...
							foreach ( $rows as $row )
							{
								if($i % 2 === 0) {
									...
									echo '<img src="'.get_site_url().'/wp-content/plugins/visitors-voice/assets/open_in_new_window.png" class="open-in-new-ico open-hitlist tooltip" title="'.__('Open the page in new window','visitorsvoice').'">';
									echo '</a>';
									echo '</td><td>';
...
?>
					...
					?>
					<label for="visitorsvoice_meta_box_text"><?php _e('Mark all these keyword suggestions as fixed', 'visitorsvoice').'<br>';?>			
					</fieldset>
<?php					
				}
			}
...
?>			
			<div class="metabox-tabs-div">
				...
				<ul class="metabox-tabs" id="metabox-tabs">
					<li class="active tab1"><a class="active" href="javascript:void(null);"><?php _e("Incoming", "visitorsvoice"); ?></a></li>
					<li class="tab2"><a href="javascript:void(null);"><?php _e("Outgoing", "visitorsvoice"); ?></a></li>
						...
						if(count($searchterms_to_url)>0)
						{
							foreach( $searchterms_to_url as $record ) {
								if($i % 2 === 0) {
									...
									echo '<td><a href="'.str_replace('##SEARCHTERM##', $record["SearchTerm"], $this->internal_search_url).'" target="_blank">'.$record["SearchTerm"].'<img src="'.get_site_url().'/wp-content/plugins/visitors-voice/assets/open_in_new_window.png" class="open-in-new-ico open-hitlist tooltip" title="'.__('Open the page in new window','visitorsvoice').'"></a></td>';
									echo '<td>'.$record["Count"].'</td>';
									echo '<td>'.$record["User"].'</td>';
									echo '<td><a href="http://service.visitorsvoice.com/report/Analysis?searchterm='.$record["SearchTerm"].'&token=view-'.$visitorsvoiceplugin->get_api_key().'" target="_blank">'.__("Analyze", "visitorsvoice").'<img src="'.get_site_url().'/wp-content/plugins/visitors-voice/assets/open_in_new_window.png" class="open-in-new-ico open-hitlist tooltip" title="'.__('Open the page in new window','visitorsvoice').'"></a></td>';
									echo '</tr>';
								} else {
					...
					<table class="display" id="outgoing_searchterms_table">
						<thead>
							<tr>
								...
								<th><?php _e("Count", "visitorsvoice"); ?></th>
								<th><?php _e("Users", "visitorsvoice"); ?></th>
								<th><?php _e("CTR (%)", "visitorsvoice"); ?></th>
								<th><?php _e("Analyze search terms", "visitorsvoice"); ?></th>
							</tr>
						</thead>
						<tbody>
...
<?php		
						...
						if(count($keywords_to_url)>0)
						{
							foreach( $keywords_to_url as $record ) {
								if($i2 % 2 === 0) {
									echo '<tr class="even gradeX">';
									echo '<td><a href="'.str_replace('##KEYWORD##', $record["Keyword"], $this->google_url).'" target="_blank">'.$record["Keyword"].'<img src="'.get_site_url().'/wp-content/plugins/visitors-voice/assets/open_in_new_window.png" class="open-in-new-ico open-hitlist tooltip" title="'.__('Open the page in new window','visitorsvoice').'"></a></td>';
									echo '<td>'.$record["Count"].'</td>';
									echo '<td>'.$record["User"].'</td>';
									echo '</tr>';
								} else {
