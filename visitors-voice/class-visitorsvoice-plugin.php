	...
	class VisitorsVoicePlugin {
		...
		*/			
		public function get_new_page_suggestions_list() {
			...
			try {
				$resp = json_decode(utf8_encode($this->client->get_new_page_suggestions_list($this->max_nr_prioritized_page)), true);
		...
		*/			
		public function get_new_page_suggestions($permalink) {
			...
			try {
				...
				$resp = json_decode(utf8_encode($this->client->get_new_page_suggestions($permalink)), true);
		...
		*/			
		public function get_url_context($url) {
			...
			if(!empty($url)){
				try {
					$url_context = json_decode(utf8_encode($this->client->get_url_context($url, $this->url_context_days_back)), true);
		...
		*/
		public function Visitorsvoice_search_terms_list() {
			...
			if($this->check_configured()) {
				echo '<iframe src="http://service.visitorsvoice.com/Report/Searchterms?token=VIEW-'.$this->client->get_api_key().'" width="95%" height="950" frameBorder="0"></iframe>';
			} else {
				$this->display_non_authorized();
			}
		}
		...
		*/
		public function Visitorsvoice_total_search_quality() {
			if( ! is_admin() )
				return;
			if($this->check_configured()) {
				echo '<iframe src="http://service.visitorsvoice.com/Report/Overview?token=VIEW-'.$this->client->get_api_key().'" width="95%" height="950" frameBorder="0"></iframe>';
			} else {
				$this->display_non_authorized();
			}
		}
		...
		*/
		public function display_non_authorized() {
			...
			echo '</br></br>';
			echo __('If you don\'t have a Visitors Voice account, use our <a href="http://en.visitorsvoice.com/sign-up-for-free-trial/" target="_new"> free trial</a> or apply for a <a href="http://en.visitorsvoice.com/apply-for-a-developer-account/" target="_new"> developer account</a>.', 'visitorsvoice');
			echo '</br></br>';
